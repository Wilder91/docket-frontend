...
function HomePage() {
  ...
  return (
    ...
    <h1>{user.name}'s Projects</h1>   <button className='normal' onClick={openModal}>Add Project</button>
      <Modal
        ariaHideApp={false}
        className='modal'
        isOpen={modalIsOpen}
    ...
    <ul>
      ...
      <Modal
        ...
        contentLabel="Example Modal"
      >
       <editProject setProjects={setProjects} />
      </Modal>
        ...
        <br></br>
        {milestones.map(milestone => (<li key={milestone.id}>  <b>{milestone.name}</b> 
         <br></br> {milestone.description}<br></br> Due Date: {milestone.due_date} <br /> 
